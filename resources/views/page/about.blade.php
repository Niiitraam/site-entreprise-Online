<section class="div-about" id="about-me">
    <h3 class="text1-about bleu"><span class="numero-titre">01</span> About me</h3>
    <div class="skills">
        <div class="image-photo-about">
            <!-- <img src="{{secure_asset('image/photo-martin.webp')}}" alt="photo martin" class="photo-martin"> -->
            <picture>
                <source srcset="{{secure_asset('image/photo-martin.webp')}}" alt="photo martin" class="photo-martin">
                <source srcset="{{secure_asset('image/photo-martin.jpg')}}" alt="photo martin" class="photo-martin">
            </picture>
            <div class="rectangle-photo"></div>
        </div>
    </div>
    <div class="skills">
        <h4 class="titre-skills">Programming languages & others</h4>
        <div class="section-tools">
            <p class="texte-about">HTML</p>
            <p class="texte-about">CSS</p>
            <p class="texte-about">Javascript</p>
            <p class="texte-about">Greensock</p>
            <p class="texte-about">Laravel</p>
            <p class="texte-about">Github</p>
            <p class="texte-about">Wordpress</p>
            <p class="texte-about">Elementor</p>
        </div>
    </div>
    <div class="skills">
        <h4 class="titre-skills">Tools</h4>
        <div class="section-tools">
            <p class="texte-about">figma</p>
            <p class="texte-about">heroku</p>
            <p class="texte-about">Visual Studio Code</p>
            <p class="texte-about">illustrator</p>
        </div>
    </div>
    <div class="skills">
        <h4 class="titre-skills">Working experience</h4>
        <div class="section-tools">
            <p class="texte-about">web designer<br>Independent</p>
            <p class="texte-about-s">2022 - Present</p>
            <p class="texte-about">Web Designer<br> at Skydoo Brussels</p>
            <p class="texte-about-s">2021 - 2022</p>
            <p class="texte-about">Independent</p>
            <p class="texte-about-s">2020 - 2021</p>
        </div>
    </div>
</section>